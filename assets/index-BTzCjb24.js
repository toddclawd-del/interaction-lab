import{j as t}from"./three-vendor-CgYOwSjb.js";import{r as a}from"./react-vendor-DtyY8K5c.js";const A=12;function D(R={}){const{enableTilt:o=!0,enableSpotlight:c=!0,tiltStrength:b=.5,maxTilt:v=A}=R,f=a.useRef(null),d=a.useRef(null),[u,g]=a.useState({isHovered:!1,isPressed:!1,isFocused:!1,spotlightX:"50%",spotlightY:"50%",rotateX:0,rotateY:0}),[p,S]=a.useState(!1);a.useEffect(()=>{const e=window.matchMedia("(prefers-reduced-motion: reduce)");S(e.matches);const h=s=>S(s.matches);return e.addEventListener("change",h),()=>e.removeEventListener("change",h)},[]);const x=a.useCallback((e,h)=>{if(!f.current)return;const s=f.current.getBoundingClientRect(),l=(e-s.left)/s.width*100,F=(h-s.top)/s.height*100,w=Math.max(0,Math.min(100,l)),m=Math.max(0,Math.min(100,F)),G=(e-s.left)/s.width-.5,k=(h-s.top)/s.height-.5,y=v*b,E=p||!o?0:k*-y,P=p||!o?0:G*y;g(I=>({...I,spotlightX:c?`${w}%`:"50%",spotlightY:c?`${m}%`:"50%",rotateX:E,rotateY:P}))},[o,c,b,v,p]),B=a.useCallback(e=>{d.current&&cancelAnimationFrame(d.current),d.current=requestAnimationFrame(()=>{x(e.clientX,e.clientY)})},[x]),T=a.useCallback(()=>{g(e=>({...e,isHovered:!0}))},[]),C=a.useCallback(()=>{d.current&&(cancelAnimationFrame(d.current),d.current=null),g(e=>({...e,isHovered:!1,isPressed:!1,rotateX:0,rotateY:0}))},[]),M=a.useCallback(()=>{g(e=>({...e,isPressed:!0}))},[]),i=a.useCallback(()=>{g(e=>({...e,isPressed:!1}))},[]),z=a.useCallback(()=>{g(e=>({...e,isFocused:!0,spotlightX:"50%",spotlightY:"50%"}))},[]),j=a.useCallback(()=>{g(e=>({...e,isFocused:!1}))},[]);a.useEffect(()=>()=>{d.current&&cancelAnimationFrame(d.current)},[]);const n={onMouseMove:B,onMouseEnter:T,onMouseLeave:C,onMouseDown:M,onMouseUp:i,onFocus:z,onBlur:j},W={"--spotlight-x":u.spotlightX,"--spotlight-y":u.spotlightY,"--rotate-x":`${u.rotateX}deg`,"--rotate-y":`${u.rotateY}deg`};return{ref:f,state:u,handlers:n,style:W,prefersReducedMotion:p}}const Y={subtle:{spotlightGradient:`radial-gradient(
